...
from common.common_permission.permission import get_login_scope
...
class OnlineService:
    ...
    @staticmethod
    async def list_online(page: int = 1, page_size: int = 10, keyword: str = None,
        ...
        数据权限：非管理员仅保留本人及可见部门内的在线用户。
        """
        scope_dept_ids = get_login_scope(login_user) if login_user is not None else None
        me = login_user.get("user_id") if login_user is not None else None
        items = []
        async for key in client.client.scan_iter(match=PREFIX_LOGIN + "*", count=100):
            payload = await client.get(key, to_dict=True)
            if not isinstance(payload, dict):
                continue
            # 数据权限过滤：scope_dept_ids=None 不限；否则仅本人或部门命中的在线会话可见
            if login_user is not None and scope_dept_ids is not None:
                online_uid = payload.get("user_id")
                if online_uid != me and payload.get("dept_id") not in scope_dept_ids:
                    continue
