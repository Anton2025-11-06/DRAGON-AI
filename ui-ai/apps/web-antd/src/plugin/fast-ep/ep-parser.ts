...
export function parserSchema(obj: any) {
 ...
 */
export function findObjectsWithField(obj: any, field: string): any[] {
  const result: any[] = [];
  ...
  function traverse(node: any) {
    if (_.isPlainObject(node)) {
      ...
      // 遍历对象的所有属性
      _.forOwn(node, (value: any) => traverse(value));
    } else if (_.isArray(node)) {
      // 遍历数组中的每个元素
      _.forEach(node, (item: any) => traverse(item));
    }
  }
  traverse(obj);
  return result;
 ...
 */
export function fieldToFastCrud(fieldList: any[]): Record<string, any> | null {
 ...
 */
export function toFastCrudField(field: any) {
